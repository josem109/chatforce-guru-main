...
const Conversations: React.FC = () => {
  const [conversations, setConversations] = useState<Conversation[]>(generateMockConversations(20));
  ...
  const uniqueUsers = Array.from(new Set(conversations.map(conv => conv.userId)));
  ...
  const filteredConversations = conversations.filter(conversation => {
    ...
    const matchesUser = userFilter === 'all' || conversation.userId === userFilter;
  ...
  const handleExport = () => {
    try {
      ...
      a.href = url;
      a.download = `chatforce-conversations-${new Date().toISOString().split('T')[0]}.json`;
    ...
    <div className="space-y-6">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border/30">
              {filteredConversations.map((conversation) => (
                <tr key={conversation.id} className="hover:bg-secondary/20 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-muted-foreground">
                    <div className="flex items-center">
                      ...
                      {formatDistance(conversation.lastMessageDate, new Date(), {
                        addSuffix: true,
                        locale: es
                      })}
                    </div>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Conversación con {selectedConversation?.userName}</DialogTitle>
            <DialogDescription>
            ...
            {selectedConversation?.messages.map((message, index) => (
              ...
              >
                ...
                >
                  ...
                  <div 
                    className={`text-xs ${
                      message.isUser ? 'text-chat-user-foreground/70 text-right' : 'text-chat-bot-foreground/70'
                    }`}
                  >
